...
int main()
{
	...
	{{0, 0},{1, 0},{1, 1},{1, 2}}, {{0, 0},{1, 0},{2, 0},{2, 1}}, {{0, 0},{0, 1},{0, 2},{1, 0}}, {{0, 0}, {0, 1}, {1, 1}, 
	{2, 1}}, {{1, 0},{1, 1},{1, 2},{0, 2}}, //l_shapes
		...
		for(int add_m = 0; add_m < m; add_m++)
		{
			for(int i = 0; i < 19; i++)
			{
				for(int j  = 0; j < 4; j++)
				{
					...
					int x = shapes[i][j][0];
					int y = shapes[i][j][1];
					if (x + add_n >= 0 && y + add_m >= 0 && x + add_n < n && y + add_m < m)
					{
						sum += papar[x + add_n][y + add_m];
